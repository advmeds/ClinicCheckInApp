...
data class SessionRequest(
    ...
    companion object {
        private fun eventDataToRequest(eventData: EventData): EventDataRequest {
            val requestParam = mutableMapOf<String, String>()

            for ((key, value) in eventData.params) {
                requestParam[key] = Converter.anyToString(value)
            }
        ...
        fun fromMapToSessionRequest(
            ...
            return sessions.map { session ->
                SessionRequest(
                    ...
                    session.deviceId,
                    sessionMap[session.id]?.map { eventData ->  eventDataToRequest(eventData) }
                )
            }
        }
    }
